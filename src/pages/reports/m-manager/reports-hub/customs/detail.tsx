...
import { CustomsDetailColumns } from "./columns";
...
export default function CustomsDetailPage() {
  ...
  const handleExport = () => {
    ...
    window.open(
      `${baseUrl}/bojxona/report/excel?year=${year}&month=${month}&customsId=${customsId}`,
      "_blank"
    );
  };

  const cards = [
    { title: "Olingan", price: formatPrice(customs?.owed || 0) },
    { title: "To'langan", price: formatPrice(customs?.given || 0) },
    { title: "Qolgan", price: formatPrice(customs?.totalDebt || 0) },
  ];
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-[22px] font-bold text-foreground">
                {formatPrice(customs?.totalDebt || 0)} $
              </p>
            </div>
          </div>
        </div>
      ...
      {totals && (
        <div className="bg-[#EEEEEE] flex">
          ...
          <p className="p-[20px] border-border border-r text-[14px] w-full text-[#E38157]">
            To'langan: {formatPrice(totals.total_expense || 0)} $
          </p>
