...
import { LogisticsDetailColumns } from "./columns";
...
export default function LogisticsDetailPage() {
  ...
  const handleExport = () => {
    ...
    window.open(
      `${baseUrl}/logistics/report/excel?year=${year}&month=${month}&logisticsId=${logisticsId}`,
      "_blank"
    );
  };

  const cards = [
    { title: "Olingan", price: formatPrice(logistics?.owed || 0) },
    { title: "To'langan", price: formatPrice(logistics?.given || 0) },
    { title: "Qolgan", price: formatPrice(logistics?.totalDebt || 0) },
  ];
          ...
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-[22px] font-bold text-foreground">
                {formatPrice(logistics?.totalDebt || 0)} $
              </p>
            </div>
          </div>
        </div>
