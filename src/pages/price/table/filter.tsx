...
export default function PriceFilter({ children }: { children?: React.ReactNode }) {
  ...
  return (
    <div className="flex items-center gap-[4px] shrink-0 mb-[10px]">
      ...
      {/* Tabs */}
      <div className="flex items-center gap-[2px] bg-white rounded-[8px] px-[4px] h-[42px]">
        <Button
          variant="ghost"
          size="sm"
          className={`h-[34px] rounded-[6px] text-[13px] ${pathName === "/price" ? "bg-[#F5F5F5] font-medium" : ""}`}
        ...
        <Button
          variant="ghost"
          size="sm"
          className={`h-[34px] rounded-[6px] text-[13px] ${pathName === "/discount" ? "bg-[#F5F5F5] font-medium" : ""}`}
